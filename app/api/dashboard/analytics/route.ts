...
export async function GET(request: Request) {
  ...
  const [
    ...
    // Messages with timestamps for hourly distribution
    prisma.message.findMany({
      where: sessionTenantFilter,
      select: { createdAt: true },
    }),
    // First user message per session for popular topics
    prisma.chatSession.findMany({
      where: tenantFilter,
      select: {
        messages: {
  ...
  // Most active hours (24h distribution)
  ...
  // Popular first messages / common topics
  ...
  // Count frequency of first messages (trimmed, lowercased)
